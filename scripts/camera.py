...
class Camera_output:
    ...
    def data_point(self):
        ...
        self.last_data_trans, self.last_data_rot = self.listener.lookupTransform("/map", "tilt_link", rospy.Time(0))
    ...
    def data_local(self, w, h):
        xt, yt, zt = self.last_data_arr['x'][w, h], self.last_data_arr['y'][w, h], self.last_data_arr['z'][w, h]
        return np.array([xt, yt, zt])


    def data_global(self, xt, yt, zt):
