...
class Road:
    def __init__(self, fx, fy, live = True):
        ...
        self.road_loc_thrs = 2             #if avg x of road > thrs then road is on right side of frame
        self.curr_h_wrtroad = 10
        self.curr_x_road = 0
        self.curr_x_factor = 4
        self.const_x_factor = -11
        self.d_yaw_drone = 0
        # self.d_yaw_drone_linear = 0
        self.slope_threshold = 5
        self.x_thrs = 14
        self.wt_factor_yaw = 0.95
        self.wted_yaw = 0
    ...
    def center_finder_live(self, h_x, w_y):
        h_ = np.array((h_x + 0.5)*self.k, dtype = int)
        w_ = np.array((w_y + 0.5)*self.k, dtype = int)
        return self.cam.data_local(w_, h_)
    ...
    def cal_nxt_waypoint(self):
        ...
        # print(n)
        if len(self.curr_s0) > 1 and not n == 0:
            self.p0_found = True
            ...
            self.p0_found = False
        if len(self.curr_s1) > 1 and not n == 0:
            self.p1_found = True
        ...
        if self.p0_found and self.p1_found:
            if np.sum(np.abs(self.p0[1] - self.p1[1])) < self.x_thrs:
                ...
                print('both edge found')
            elif len(self.curr_s0) > len(self.curr_s1):
                self.waypt = self.p0.copy()
                self.waypt[0] -=self.road_width
                # self.waypt[0] -= (self.curr_x_road*self.curr_x_factor + self.const_x_factor)
            else:
            ...
            # pass                #work with p0
            self.waypt = self.p0.copy()
            self.waypt[0] += (self.curr_sense*self.road_width)

        else:
            self.waypt_not_found = True
            print('Unable to compute waypt')


        self.waypt_global = self.cam.data_global(self.waypt[0], self.waypt[1], self.waypt[2])
    ...
    def cal_dyaw(self):
        ...
        if self.ts_e0 or self.ts_e1:
            if avg_slope < 0:
                self.d_yaw_drone = -1*self.d_yaw_drone
            self.wt()

    def wt(self):
        self.wted_yaw = self.wted_yaw*(1 - self.wt_factor_yaw) + self.d_yaw_drone*self.wt_factor_yaw
