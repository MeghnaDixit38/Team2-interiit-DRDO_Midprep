...
from camera import Camera_output
...
class Mapping():
    ...
    def cnvt_global(self):
        ...
        if len(self.curr_s0) > 0:
            temp = self.road.cam.data_global(self.curr_s0[:, 0], self.curr_s0[:, 1], self.curr_s0[:, 2])
            self.curr_s0 = np.array([temp[0], temp[1], temp[2]]).T

        if len(self.curr_s1) > 0:
            temp = self.road.cam.data_global(self.curr_s1[:, 0], self.curr_s1[:, 1], self.curr_s1[:, 2])
    ...
    def main(self):
        ...
        #     print(self.road.p0)
        #     print(self.road.cam.data_global(self.road.p0[0], self.road.p0[1], self.road.p0[2]))
        # if self.road.p1_found:
        #     print(self.road.p1)
        #     print(self.road.cam.data_global(self.road.p1[0], self.road.p1[1], self.road.p1[2]))
        # print(self.road.curr_h_wrtroad, self.road.curr_sense, self.road.curr_x_road, self.road.d_yaw_drone)
        self.return_stuff = [self.waypt_global, self.road.wted_yaw, self.road.d_yaw_drone]
        print('way point wrt drone ' + str(self.road.waypt))
        # print('Current yaw ' + str(map.iris.curr_yaw))
        # print('waypt wrt drone'+str(self.road.waypt[0]))

        # print(self.road.curr_s0)
        ...
        # x, y = self.dy, self.road.waypt[1]
        # self.x_out, self.y_out = np.matmul(rotation_matrix, np.array([x, y]))
        # print(self.iris.curr_yaw)
        # print('waypt'+str(self.road.waypt))
        # print('waypt global' + str(self.road.waypt_global))
        if self.road.road_not_found or self.road.waypt_not_found:
            np.save('road_2.npy', np.array(self.mid_points))
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # (map.return_stuff[0] == np.nan)
            # break
        print(map.return_stuff)
        map.return_stuff[0] =  np.nan_to_num(map.return_stuff[0], nan = 0)
        mav.gotopose_yaw_1(map.return_stuff[0][0], map.return_stuff[0][1] ,map.return_stuff[0][2] + 19, mav.curr_yaw + map.road.wted_yaw)
    # mav.gotopose_yaw_1(map.x_out, map.y_out, map.return_stuff[0][2] + 19, mav.curr_yaw + map.return_stuff[1])
    # r.sleep()
